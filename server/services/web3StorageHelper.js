...
export async function createAuthenticatedClient() {
  try {
    ...
    if (spaceDID !== 'did:key:z6MkqqowtdccpY2kJxXcfSr27xCKKCL1eFL6wSBZq1ATUgzo') {
      console.warn('⚠️ Environment variable space DID doesn\'t match expected space');
    }
    
    // First authenticate with email
    if (process.env.WEB3_AUTH_EMAIL) {
      try {
        console.log(`Authenticating with email: ${process.env.WEB3_AUTH_EMAIL}`);
        ...
        if (currentSpace.did() !== spaceDID) {
          console.error('❌ Failed to set the correct space! Using fallback options...');
        }
    ...
    console.log('All Web3Storage authentication methods failed, falling back to local storage');
    return null;
  } catch (error) {
