...
router.get("/datasets/discover", async (req, res) => {
  try {
    ...
    const datasets = Object.entries(filesByCategory).map(
      ([category, categoryFiles]) => ({
        ...
        dataTypes: [
          ...new Set(
            categoryFiles.flatMap((file) => file.dataTypes).filter(Boolean)
          ),
        ],
        tags: [
          ...new Set(
            categoryFiles.flatMap((file) => file.tags).filter(Boolean)
          ),
        ],
