...
class Web3Service {
  ...
  // Retrieve and decrypt a dataset by CID.
  //
  // The symmetric key is fetched from the server's key custody endpoint,
  // which authorizes the caller (must be the registered owner or hold a
  // server-side access grant). Callers no longer pass the key directly —
  // pre-fix versions of this method accepted (cid, encryptionKey) and the
  // key was read off-chain from HealthData.encryptionKey. That field has
  // been removed from the contract.
  async retrieveHealthData(cid) {
    try {
      ...
      let encryptionKey;
      try {
        const keyResp = await apiService.get(`/data/keys/${cid}`);
        encryptionKey = keyResp?.dataKey;
      } catch (keyErr) {
        await hipaaComplianceService.createAuditLog("KEY_FETCH_FAILURE", {
          cid: cid.toString(),
          error: keyErr.message,
          userInfo: hipaaComplianceService.getUserInfo(),
        });
        throw new Web3Error(
          "Not authorized to decrypt this dataset, or key not found on server.",
          "KEY_FETCH_FAILED",
          { cid: cid.toString(), originalError: keyErr.message }
        );
      }

      if (!encryptionKey) {
        throw new Web3Error(
          "Server returned no key for this dataset.",
