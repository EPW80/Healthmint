...
// Fetch saved subsets for the current user
const useFilteredSubset = () => {
  ...
  const [selectedDatasetForFiltering, setSelectedDatasetForFiltering] =
    useState(null);
  const [savedSubsets, setSavedSubsets] = useState([]);
  const [purchaseInProgress, setPurchaseInProgress] = useState(false);
  const [activeSubsetId, setActiveSubsetId] = useState(null);

  // Load saved subsets for the current user
  const startFilteringDataset = useCallback(
  ...
  // Purchase a subset
  const handleSubsetCreated = useCallback(
    (subsetData) => {
  ...
  // Purchase a subset
  const handleSubsetPurchase = useCallback(
    async (subsetData) => {
      try {
  ...
  // Get saved subsets for a specific dataset
  const getDatasetSubsets = useCallback(
    (datasetId) => {
      return savedSubsets.filter(
        (subset) => subset.parentDatasetId === datasetId
      );
    },
    [savedSubsets]
  );
