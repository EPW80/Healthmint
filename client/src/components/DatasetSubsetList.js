    ...
    >
      <div className="bg-indigo-50 border-b border-indigo-100 p-4">
        ...
        <p className="text-sm text-indigo-600 mt-1">
          {subsets.length} filtered subset{subsets.length !== 1 ? "s" : ""}{" "}
          available
        </p>
      </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  {subset.filters?.timeRange?.start &&
                    subset.filters?.timeRange?.end && (
                  ...
                  {subset.filters?.gender &&
                    subset.filters.gender !== "all" && (
                  ...
                  {(subset.filters?.ageRange?.min ||
                    subset.filters?.ageRange?.max) && (
                    <span className="inline-flex items-center px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-800">
                      <Tag size={12} className="mr-1" />
                      Age: {subset.filters.ageRange.min || "0"}-
                      {subset.filters.ageRange.max || "120"}
                    </span>
                  )}
              ...
              <div className="text-right">
                <div className="flex items-center justify-end gap-1 text-gray-500 text-sm">
                  ...
                  <span>
                    {new Date(
                      subset.createdAt || subset.created || Date.now()
                    ).toLocaleDateString()}
                  </span>
                </div>
